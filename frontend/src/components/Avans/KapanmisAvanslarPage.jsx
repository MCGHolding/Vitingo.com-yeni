...
export default function KapanmisAvanslarPage({ onBackToDashboard }) {
  const { toast } = useToast();
  const [avanslar, setAvanslar] = useState([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');

  // Load closed advances (finance approved only)
  useEffect(() => {
    loadKapanmisAvanslar();
  }, []);

  const loadKapanmisAvanslar = async () => {
    try {
      setLoading(true);
      const backendUrl = process.env.REACT_APP_BACKEND_URL || import.meta.env.REACT_APP_BACKEND_URL;
      const response = await fetch(`${backendUrl}/api/avans/kapanmis`);
      if (response.ok) {
        const data = await response.json();
        setAvanslar(data);
      } else {
        console.error('Failed to load closed advances');
      }
    } catch (error) {
      console.error('Error loading closed advances:', error);
    } finally {
      setLoading(false);
    }
  };

  const handleView = (avans) => {
    console.log('View avans:', avans);
    // This will be implemented with proper modal/navigation
  };
      ...
      <div className="p-6">
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            <CheckCircle className="h-8 w-8 text-gray-400 mx-auto mb-2 animate-spin" />
            <p className="text-gray-500">Kapanmış avanslar yükleniyor...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Kapanmış Avanslar</h1>
          <p className="text-gray-600">Finans onayından geçmiş ve kapanmış avanslar</p>
        </div>
        <Button 
          variant="outline" 
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <p className="text-sm font-medium text-gray-600">Kapanmış Avanslar</p>
      ...
      {/* Closed Advances Table */}
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Kapanmış Avanslar Listesi</CardTitle>
        </CardHeader>
        <CardContent>
          {filteredAvanslar.length === 0 ? (
            <div className="text-center py-8">
              <CheckCircle className="h-12 w-12 text-gray-400 mx-auto mb-4" />
              <p className="text-gray-500 mb-2">Kapanmış avans bulunamadı</p>
              <p className="text-sm text-gray-400">Henüz finans onayından geçmiş avans bulunmuyor.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    <th className="text-left py-2 px-3 font-medium text-gray-600 text-xs">Onay Tarihi</th>
                    <th className="text-left py-2 px-3 font-medium text-gray-600 text-xs">Onaylayan</th>
                ...
                <tbody>
                  {filteredAvanslar.map((avans, index) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-3">
                        <div className="text-gray-700 text-sm">
                          {formatDate(avans.finance_approved_at)}
                        </div>
                      </td>
                      
                      <td className="py-3 px-3">
                        <div className="text-gray-700 text-sm">
                          {avans.finance_approved_by || 'Sistem'}
                        </div>
                      </td>
                      
                      <td className="py-3 px-3">
                        <Badge className="bg-green-100 text-green-800 border-0 text-[10px] px-2 py-1">
                          Kapanmış
                        </Badge>
                      </td>
