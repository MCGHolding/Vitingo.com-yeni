...
export default function FutureFairsPage({ fairs: initialFairs, onBackToDashboard }) {
  ...
  const loadFairs = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Filter for future fairs
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        
        const futureFairs = fairsData.filter(fair => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow-sm border-b border-gray-200 px-6 py-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-green-600">Gelecek Fuarlar</h1>
              <p className="text-gray-600 mt-1">Yaklaşan ve aktif fuar etkinliklerini görüntüleyin • {totalFairs} fuar</p>
            </div>
          </div>
      ...
      <div className="px-6 py-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6">
          ...
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-700">
              <span className="text-green-600 font-bold">{filteredFairs.length}</span> fuar bulundu
            </span>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="p-4 border-b border-gray-200">
            <h3 className="text-lg font-medium text-gray-900">Gelecek Fuarlar Listesi</h3>
          </div>
          ...
          {filteredFairs.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">
                {searchTerm ? 'Arama sonucu bulunamadı' : 'Henüz gelecek fuar eklenmemiş'}
              </p>
            </div>
          ) : (
                  ...
                  {filteredFairs.map((fair, index) => (
                    ...
                    >
                      <td className="py-3 px-4">
                        <span className="font-medium text-green-600 text-sm">{index + 1}</span>
                      </td>
      ...
      {/* Modals - Same as AllFairsPageNew but omitted for brevity */}
      {/* You can copy all modals from AllFairsPageNew.jsx */}
    </div>
  );
}
