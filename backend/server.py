...
async def send_survey_invitation(
    ...
    try:
        ...
        base_url = os.environ.get('FRONTEND_URL', 'https://crm-dashboard-114.preview.emergentagent.com')
...
async def send_expense_receipt_approval_email(receipt, approval_key):
    ...
    try:
        ...
        # Create approval URL
        frontend_url = os.environ.get('FRONTEND_URL', 'https://crm-dashboard-114.preview.emergentagent.com')
        ...
        <div style="max-width: 600px; margin: 0 auto; background-color: white; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
            ...
            <div style="padding: 30px;">
                ...
                <div style="text-align: center; margin: 30px 0;">
                    ...
                    <a href="https://crm-dashboard-114.preview.emergentagent.com{pdf_link}" 
...
async def get_dashboard_stats():
    ...
    try:
        ...
        this_month_invoices = [inv for inv in invoices if str(inv.get("date", ""))[:7] == current_month]
...
@api_router.get("/library/design-templates")
async def get_design_templates(category: str = None):
    """Get all design templates"""
    try:
        query = {}
        if category:
            query["category"] = category
        templates = await db.design_templates.find(query, {"_id": 0}).sort("created_at", -1).to_list(1000)
        return templates
    except Exception as e:
        logger.error(f"Error fetching design templates: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))

@api_router.post("/library/design-templates")
async def create_design_template(name: str = Form(""), category: str = Form("cover_page"), file: UploadFile = File(...)):
    """Create a new design template with file upload"""
    try:
        ...
        await db.design_templates.insert_one(template_dict)
        return {"id": template_dict["id"], "message": "Tasarım şablonu eklendi"}
    except Exception as e:
        logger.error(f"Error creating design template: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))

@api_router.delete("/library/design-templates/{template_id}")
...
@api_router.post("/library/design-templates/seed-defaults")
async def seed_default_design_templates():
    """Seed default design templates"""
    try:
        existing = await db.design_templates.count_documents({})
        if existing > 0:
            return {"message": "Varsayılan şablonlar zaten mevcut", "count": existing}
        ...
        default_templates = [
            {
                ...
                "name": "Teklif Kapak 1",
                "category": "cover_page",
                "image_url": "https://customer-assets.emergentagent.com/job_docgen-pro-9/artifacts/5rm7sspq_Untitled%20design.jpg",
                "created_at": datetime.now(timezone.utc).isoformat()
            },
            {
                "id": str(uuid.uuid4()),
                "name": "Teklif Kapak 2",
                "category": "cover_page",
                "image_url": "https://customer-assets.emergentagent.com/job_docgen-pro-9/artifacts/wddz4erk_Untitled%20design%20%281%29.jpg",
                "created_at": datetime.now(timezone.utc).isoformat()
            },
            {
                "id": str(uuid.uuid4()),
                "name": "Teklif Kapak 3",
                "category": "cover_page",
                "image_url": "https://customer-assets.emergentagent.com/job_docgen-pro-9/artifacts/hjdqqruu_Untitled%20design%20%282%29.jpg",
                "created_at": datetime.now(timezone.utc).isoformat()
            }
        ]
        
        await db.design_templates.insert_many(default_templates)
        return {"message": "Varsayılan şablonlar eklendi", "count": len(default_templates)}
    except Exception as e:
...
# ============================================================================
# WIO BANK PDF PARSING
# ============================================================================

def parse_wio_bank_pdf(pdf_bytes: bytes) -> dict:
    """Parse Wio Bank PDF statement and extract transactions"""
    import pdfplumber
    import io
    import re
