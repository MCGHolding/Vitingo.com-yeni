...
async def send_survey_invitation(
    ...
    try:
        ...
        base_url = os.environ.get('FRONTEND_URL', 'https://fairmanager-crm.preview.emergentagent.com')
...
async def send_expense_receipt_approval_email(receipt, approval_key):
    ...
    try:
        ...
        # Create approval URL
        frontend_url = os.environ.get('FRONTEND_URL', 'https://fairmanager-crm.preview.emergentagent.com')
        ...
        <div style="max-width: 600px; margin: 0 auto; background-color: white; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
            ...
            <div style="padding: 30px;">
                ...
                <div style="text-align: center; margin: 30px 0;">
                    ...
                    <a href="https://fairmanager-crm.preview.emergentagent.com{pdf_link}" 
...
@api_router.get("/contracts/files/{file_id}")
async def download_contract_file(file_id: str):
...
async def create_template(template: ContractTemplateCreate):
    ...
    try:
        # For now, use a default user ID (will be replaced with JWT auth)
        template_dict = {
            **template.dict(),
            "id": str(uuid.uuid4()),
            "fields": [],
...
@api_router.put("/contracts/templates/{template_id}", response_model=ContractTemplate)
async def update_template(template_id: str, template: ContractTemplateCreate):
    """Update a template"""
    try:
        update_dict = {
            **template.dict(),
...
@api_router.delete("/contracts/templates/{template_id}")
...
# Add/Update Fields for Template
...
async def download_generated_contract(contract_id: str):
    """Download a generated contract"""
    try:
        contract = await db.generated_contracts.find_one({"id": contract_id})
        if not contract:
            raise HTTPException(status_code=404, detail="Sözleşme bulunamadı")
        
        # TODO: Get file from GridFS and return
        raise HTTPException(status_code=501, detail="Henüz uygulanmadı")
    except HTTPException:
        raise
    except Exception as e:
...
async def delete_contract(contract_id: str):
    ...
    try:
        result = await db.generated_contracts.delete_one({"id": contract_id})
        if result.deleted_count == 0:
            raise HTTPException(status_code=404, detail="Sözleşme bulunamadı")
